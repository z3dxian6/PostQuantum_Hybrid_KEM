...
def load_kdf_key():
    kdf_path = os.path.join(bob_dir, "K_final.bin")
...
def send_message(entry, chatbox):
    ...
    with open(os.path.join(alice_dir, "message_aesgcm_from_bob.bin"), "wb") as f:
        f.write(nonce)
        f.write(ciphertext)
    chatbox.insert(tk.END, "Moi (Bob) : " + entry.get() + "\n")
...
def receive_message(chatbox):
    ...
    if key is None:
        return
    msg_path = os.path.join(bob_dir, "message_aesgcm_from_alice.bin")
    if not os.path.exists(msg_path):
        messagebox.showinfo("Info", "Aucun message reçu de Alice.")
        return
    ...
    try:
        ...
        chatbox.insert(tk.END, "Alice : " + plaintext.decode() + "\n")
        os.remove(msg_path)
...
def main():
    ...
    send_btn = tk.Button(root, text="Envoyer à Alice", command=lambda: send_message(entry, chatbox))
    send_btn.pack(side=tk.LEFT, padx=(5,0), pady=(0,10))

    recv_btn = tk.Button(root, text="Lire message de Alice", command=lambda: receive_message(chatbox))
